...
class CustomPipeline(DiffusionPipeline, FromSingleFileMixin):
    ...
    def __init__(
        ...
        scheduler_value=FlowMatchEulerDiscreteScheduler()
    ):
        super().__init__()
    ...
    def infer(
        ...
        **kwargs,
        ...
        # Initialize values if return_value is True
        if return_value:
            if noise_seed:
                value_generator = torch.Generator(device=device).manual_seed(noise_seed)
            else:
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                ...
                for i, t in enumerate(timesteps):
                    ...
                    noise_pred = self.transformer(
                        ...
                        value_store_buffer=value_store_buffer,
                        value_states_buffer=value_state_buffer,
                    ...
                    if value_store_buffer and return_action:
                        value_state_buffer = noise_pred["value_states_buffer"]
                    ...
                    if return_value:
                        ...
                        # Use the same scheduler as action for value (flow matching)
                        values = self.scheduler_value.step(value_noise_pred, t, values, return_dict=False)[0]
