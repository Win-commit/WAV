...
class Inferencer:
    ...
    def validate(
        ...
        for i_validation in range(n_validation):
            ...
            self.val_dataloader.dataset.fix_epiidx = 1 

            if self.args.return_action:
                self.val_dataloader.dataset.fix_sidx = 0
                self.val_dataloader.dataset.fix_mem_idx = [0 for _ in range(self.args.data['train']['n_previous'])]
            ...
            if self.args.return_action:
                ...
                fig, axes = plt.subplots(10, 3, figsize=(20, 28), sharex=True)
                axes = axes.flatten()
            ...
            for i_chunk_action in range(n_chunk_action):
                ...
                # print(f"image shape: b={b}, c={c}, v={v}, t={t}, h={h}, w={w}")

                # 
                # from torchvision.utils import save_image
                # debug_dir = '/defaultShare/Genie-Envisioner/web_infer_utils/debug_images'
                # os.makedirs(debug_dir, exist_ok=True)
                # 
                # for view_idx in range(v):
                #     for time_idx in range(t):
                #         img = image[0, :, view_idx, time_idx]  # (c, h, w)
                #         save_image(img, f"{debug_dir}/view{view_idx}_t{time_idx}.jpg")
